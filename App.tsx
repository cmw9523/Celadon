...
import { analyzeVibe, getWeatherForLocation, suggestLocations } from './geminiService';
import { Goal, JournalEntry, Sticker, StickerStyle } from './types';
...
const CeladonPotteryIcon = ({ isAnalyzing }: { isAnalyzing: boolean }) => (
  <div
    className={`relative cursor-pointer transition-all duration-700 hover:scale-110 active:scale-95 group ${
  ...
  >
    ...
    >
      ...
      <path
        d="M60 110C85 110 108 92 108 62C108 38 92 20 75 14H45C28 20 12 38 12 62C12 92 35 110 60 110Z"
        fill="url(#jadeGrad)"
        stroke="#7A8D7E"
        strokeWidth="0.8"
      />
      <path
        d="M42 14C42 10 48 8 60 8C72 8 78 10 78 14"
        stroke="#7A8D7E"
        strokeWidth="2"
        strokeLinecap="round"
      />
      <path
        d="M35 30C28 38 25 48 25 60"
        stroke="white"
        strokeWidth="4"
        strokeLinecap="round"
        strokeOpacity="0.15"
      ...
      {isAnalyzing && (
        <circle cx="60" cy="62" r="45" fill="white" fillOpacity="0.2" className="animate-ping" />
      )}
    </svg>
...
const App: React.FC = () => {
  ...
  const [goals, setGoals] = useState<Goal[]>(() => {
    ...
    return saved
      ...
      : [
          {
            id: '1',
            text: '5382 Kenosha Reno',
            completed: false,
            tasks: [
              { id: 't1', text: 'Window specs', completed: true },
              { id: 't2', text: 'Material order', completed: false },
            ],
          },
        ];
  });
  ...
  // Persist states
  useEffect(() => {
    localStorage.setItem('celadon_entries', JSON.stringify(entries));
  }, [entries]);

  useEffect(() => {
    localStorage.setItem('celadon_goals', JSON.stringify(goals));
  }, [goals]);

  useEffect(() => {
    localStorage.setItem('celadon_studio', JSON.stringify(studioStickers));
  }, [studioStickers]);

  useEffect(() => {
    localStorage.setItem('celadon_creativity', creativityNote);
  }, [creativityNote]);
  ...
  const handleAuth = () => {
    ...
    if (authMode === 'signup') {
      ...
      const newUser: User = { id: Date.now().toString(), email: authEmail, passwordHash: authPassword };
        ...
        <div className="max-w-md w-full kinfolk-card p-12 text-center">
          ...
          <div className="space-y-6 text-center">
            <input
              ...
              className="auth-input w-full py-2 bg-transparent text-center"
            />
            ...
            <button
              onClick={handleAuth}
              className="w-full py-4 bg-black text-white text-[10px] uppercase tracking-widest hover:bg-gray-800 transition-colors"
              type="button"
            >
            ...
            <button
              ...
              className="text-[10px] uppercase text-gray-400 hover:text-black mt-4"
              type="button"
            >
  ...
  return (
    <div className="min-h-screen w-full max-w-md mx-auto px-4 py-6">
      {/* Floating Save Button (only once) */}
      <div className="fixed bottom-5 right-5 z-50">
        <button onClick={handleSave} type="button" aria-label="Save entry">
      ...
      <div className="space-y-10">
        ...
        <div className="kinfolk-card p-5 relative overflow-hidden">
                ...
                <div className="absolute top-full left-0 w-full bg-white shadow-2xl rounded-b-xl z-50 border border-gray-50 overflow-hidden mt-1">
                  {locationSuggestions.map((s, idx) => (
                    <button
                      ...
                      className="w-full text-left px-4 py-3 text-[9px] uppercase tracking-widest text-gray-500 hover:bg-[var(--celadon-brand)] hover:text-white transition-colors border-b border-gray-50 last:border-0"
                      type="button"
                    >
                      {s}
                    </button>
          ...
          <textarea
            ...
            className="w-full min-h-[260px] bg-transparent border-none focus:ring-0 serif text-2xl placeholder-gray-100 resize-none outline-none leading-relaxed"
            placeholder="What occupies your mind?"
          />

          {(photos.length > 0 || activeStickers.length > 0) && (
            <div className="flex flex-wrap gap-6 mt-8 items-end min-h-[80px]">
              {photos.map((p, i) => (
                <img key={i} src={p} className="w-24 h-24 object-cover rounded-sm grayscale shadow-sm" />
              ))}
              {activeStickers.map((s) => (
                <div key={s.id} className={`sticker-base style-${s.style} w-24 h-24`}>
          ...
          <div className="flex justify-between items-center mt-8 pt-6 border-t border-gray-50">
            <button
              ...
              className="flex items-center gap-2 text-gray-300 hover:text-black transition-colors group"
              type="button"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {entries
            .filter((e) => e.sharedWith.includes(currentUser.id))
            .map((entry) => (
              <div key={entry.id} className="kinfolk-card p-8 relative group">
                ...
                <div className="mt-6 pt-4 border-t border-gray-50">
                  <span className="text-[10px] text-gray-300 italic">
                    {entry.location || 'Soul realm'} • {entry.vibe}
                  </span>
                </div>
              </div>
            ))}
        ...
        <div className="kinfolk-card p-8">
                ...
                <div key={goal.id} className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <span
                        className={`sans text-[11px] uppercase tracking-widest font-bold ${
                          goal.completed || progress === 100 ? 'text-gray-200' : 'text-black'
                        }`}
                      >
                        {goal.text}
                      </span>
                  ...
                  <div className="pl-7 space-y-2.5 border-l-2 border-gray-50 ml-2 mt-2">
                    {goal.tasks.map((task) => (
                      <div key={task.id} className="flex items-center justify-between text-[10px] text-gray-400 group/task">
                        ...
                        >
                          <i
                            className={`fa-solid transition-colors ${
                              task.completed
                                ? 'fa-check-circle text-[var(--celadon-brand)]'
                                : 'fa-circle-notch text-gray-100 group-hover/task:text-gray-300'
                            }`}
                          ></i>
                          <span
                            className={`transition-all ${
                              task.completed ? 'line-through text-gray-200 opacity-60' : 'group-hover/task:text-gray-700'
                            }`}
                          >
                            {task.text}
                          </span>
                        ...
                        <button
                          onClick={() =>
                            setGoals((prev) =>
                              prev.map((g) => (g.id === goal.id ? { ...g, tasks: g.tasks.filter((t) => t.id !== task.id) } : g))
                            )
                          }
                      ...
                      <input
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') {
                            ...
                            setGoals((prev) =>
                              prev.map((g) =>
                                g.id === goal.id ? { ...g, tasks: [...g.tasks, { id: Date.now().toString(), text: v, completed: false }] } : g
                              )
                            );
          ...
          <button
            ...
            className="w-full mt-12 py-3 border border-dashed text-[10px] uppercase text-gray-300 hover:text-black hover:border-gray-900 transition-all font-medium tracking-widest"
            type="button"
          >
            + New Manifestation
          </button>
        ...
        <div className="kinfolk-card p-8 border-2 border-dashed border-gray-100">
            ...
            <div className="grid grid-cols-6 gap-2 h-40 overflow-y-auto custom-scroll pr-2 bg-gray-50/50 p-2 rounded-lg">
              {EMOJI_LIBRARY.map((emoji) => (
                <button
                  ...
                  className="text-xl hover:scale-125 transition-transform flex items-center justify-center h-8 w-8"
                  type="button"
                >
                  {emoji}
                </button>
          ...
          <div className="flex gap-2 mb-6 p-1 bg-gray-50 rounded-lg">
            {(['lift', 'polaroid', 'graphic'] as StickerStyle[]).map((style) => (
              <button
                ...
                className={`flex-1 py-1.5 text-[8px] uppercase tracking-widest rounded-md transition-all ${
                  selectedStyle === style ? 'bg-white shadow-sm text-black font-bold' : 'text-gray-400 hover:text-gray-600'
                }`}
                type="button"
              >
                {style}
          ...
          <button
            ...
            className="w-full py-3 bg-gray-900 text-white text-[9px] uppercase tracking-[0.3em] hover:bg-black shadow-md transition-shadow"
            type="button"
          >
            Upload Image
          </button>
          ...
          <div className="grid grid-cols-2 gap-4 h-48 overflow-y-auto custom-scroll mt-6 pt-4 border-t border-gray-50">
            {studioStickers.map((s) => (
              <div key={s.id} className="relative group flex justify-center">
                <div
                  onClick={() => setActiveStickers((prev) => [...prev, s])}
                  className={`sticker-base style-${s.style} w-16 h-16`}
                >
                  <img src={s.data} className="w-full h-full object-cover" />
                </div>
                <button
                  onClick={() => setStudioStickers((prev) => prev.filter((st) => st.id !== s.id))}
                  className="absolute -top-1 -right-1 w-5 h-5 bg-white text-black text-[10px] rounded-full opacity-0 group-hover:opacity-100 shadow-md flex items-center justify-center border border-gray-100"
                  type="button"
                >
